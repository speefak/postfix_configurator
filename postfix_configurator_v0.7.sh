...
#------------------------------------------------------------------------------------------------------------
 ...
 DefaultSenderMailAddress=$(awk 'NR==1 {print $2}' /etc/postfix/sasl_password | cut -d: -f1 2>/dev/null); : "${DefaultSenderMailAddress:=sender}"
 DefaultSenderMailAddressPassword=$(awk 'NR==1 {print $2}' /etc/postfix/sasl_password | cut -d: -f2 2>/dev/null); : "${DefaultSenderMailAddress:=sender_password}"
...
# DefaultSenderMailAddressPassword="sender_password"
 ...
 RequiredPackets="bash sed bsd-mailx postfix bind9-dnsutils" # awk
...
# changelog 0.7 => set as default sender email an sender passwiord from /etc/postfix/sasl_password
# changelog 0.6 => code review
